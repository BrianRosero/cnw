...
const StyledButton1 = styled(Button)(({ selected }) => ({
  backgroundColor: selected ? '#0693e3' : 'white',
  borderColor: '#fff',
  color: selected ? 'white' : '#0693e3',
  margin: '0px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(6,147,227)' : '#f0f0f0',
    borderColor: '#fff',
  },
}));
...
const StyledButton = styled(Button)(({ selected }) => ({
  ...
  margin: '1px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(6,147,227)' : '#fff',
    borderColor: '#0693e3',
  },
}));
  ...
  if (isAdminCOSMITET) {
    return (
      <StyledCard>
        ...
        {loading ? (
          <div style={{ color:'#009100', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width:'100%' }}>
            <CircularProgress />
          </div>
        ) : error ? (
          <p>{error}</p>
