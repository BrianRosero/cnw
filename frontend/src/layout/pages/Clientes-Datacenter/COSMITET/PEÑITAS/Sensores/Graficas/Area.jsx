...
const StyledButton1 = styled(Button)(({ selected }) => ({
  backgroundColor: selected ? '#009240' : 'white',
  borderColor: '#fff',
  color: selected ? 'white' : '#009240',
  margin: '0px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(0,146,64)' : '#f0f0f0',
    borderColor: '#fff',
  },
}));
...
const StyledButton = styled(Button)(({ selected }) => ({
  ...
  margin: '1px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(0,146,64)' : '#fff',
    borderColor: '#009240',
  },
}));
  ...
  if (isAdminCOSMITET) {
    return (
      <StyledCard>
        ...
        {loading ? (
          <div style={{ color:'#009240', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width:'100%' }}>
            <CircularProgress />
          </div>
        ) : error ? (
          <p>{error}</p>
