...
const MachineCard = ({ sensorId }) => {
  ...
  const metricsAdmin = [
    ...
    { label: 'Lect Disco ', value: channelData[channelIDs.diskRead]?.lastvalue || 'N/A' },
    { label: 'Uso Disco ', value: channelData[channelIDs.diskUsage]?.lastvalue || 'N/A' },
    { label: 'Escr Disco ', value: channelData[channelIDs.diskWrite]?.lastvalue || 'N/A' },
    { label: 'RAM Activa ', value: channelData[channelIDs.memoryConsumed]?.lastvalue || 'N/A' },
    { label: 'RAM Consu ', value: channelData[channelIDs.memoryActive]?.lastvalue || 'N/A' },
    { label: 'Envio Red ', value: channelData[channelIDs.networkReceived]?.lastvalue || 'N/A' },
    { label: 'Transm Red ', value: channelData[channelIDs.networkTransmitted]?.lastvalue || 'N/A' },
    { label: 'Uso Red', value: channelData[channelIDs.networkUsage]?.lastvalue || 'N/A' },
  ];
  ...
  if (isAdmin) {
    return (
      <Grid container spacing={1} style={{ width: '100%', height: '100%' }} justifyContent="center">
        {metricsAdmin.map((metric, index) => (
          <div
            key={index}
            style={{
              //flex: '0 0 0', // 🔥 No se colapsan
              minWidth: '20px', // 🔥 Tamaño mínimo de cada elemento
              textAlign: 'center',
            }}
          >
            <MetricItem>
              ...
              <Typography variant="h4" style={{ color: '#282c60' }}>
                {metric.value}
              </Typography>
            </MetricItem>
          </div>
