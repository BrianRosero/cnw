...
const Sensor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://192.168.200.155:8080/prtg-api/CAMARACC/maquina');
...
const Sensor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8080/prtg-api/ESECENTRO/COCLOESECAP02');
        console.log('Data fetched from API:', response.data);
