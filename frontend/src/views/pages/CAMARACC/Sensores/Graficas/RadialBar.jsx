...
const MachineCard = ({ sensorId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch sensor info
        const sensorResponse = await axios.get(`http://192.168.200.155:8080/sensor-alt/${sensorId}`);
        const apiSensorData = sensorResponse.data;

        // Fetch channel data
        const response = await axios.get(`http://192.168.200.155:8080/canales-alt/${sensorId}`);
        ...
        const combinedSensorData = {
          ...
          disk: (localSensorData.disk / 1024).toFixed(1),
          ram: (localSensorData.ram * 1).toFixed(2),
    ...
    <StyledCard>
      <CardContent>
        ...
        <InfoText variant="body2">
          <strong>Disco duro:</strong> {sensorInfo.disk} GB
        </InfoText>
        <InfoText variant="body2">
          <strong>RAM:</strong> {sensorInfo.ram} GB
          {/*{parseFloat(channelData[channelIDs.memoryConsumed]?.lastvalue) || 0}*/}
        </InfoText>
        <InfoText variant="body2">
