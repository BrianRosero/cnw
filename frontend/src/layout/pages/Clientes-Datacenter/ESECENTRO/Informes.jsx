...
const Informes = () => {
  ...
  const fetchPdfs = async () => {
    try {
      const response = await axios.get('http://localhost:8083/api/pdfs/COSMITET');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('http://localhost:8083/api/upload/COSMITET', formData, {
        headers: {
  ...
  const handleViewPdf = (file) => {
    setCurrentPdf(`http://localhost:8083/pdfs/COSMITET/${file}`);
    setOpenDialog(true);
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8083/api/pdfs/COSMITET/${fileToDelete}`);
      fetchPdfs();
  ...
  const createPdfAndDownload = async () => {
    ...
    await axios.post(`http://localhost:8083/createPdf`, data)
      .then(() =>
        axios.get(`http://localhost:8083/fetchPdf`, { responseType: 'blob' })
          .then((res) => {
    ...
    await axios.post(`http://localhost:8083/createPdf`, data)
      .then(() =>
        axios.get(`http://localhost:8083/fetchPdf`, { responseType: 'blob' })
          ...
          .then(() =>
            axios.post("http://localhost:8083/sendPdf", { email: emails })
              .then(response => {
