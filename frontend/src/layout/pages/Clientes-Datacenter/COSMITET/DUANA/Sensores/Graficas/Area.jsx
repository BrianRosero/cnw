...
const StyledButton1 = styled(Button)(({ selected }) => ({
  backgroundColor: selected ? '#283484' : 'white',
  borderColor: '#fff',
  color: selected ? 'white' : '#283484',
  margin: '0px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(40,52,132)' : '#f0f0f0',
    borderColor: '#fff',
  },
}));
...
const StyledButton = styled(Button)(({ selected }) => ({
  ...
  margin: '1px',
  '&:hover': {
    backgroundColor: selected ? 'rgb(40,52,132)' : '#fff',
    borderColor: '#283484',
  },
}));
  ...
  if (isAdminCOSMITET) {
    return (
      <StyledCard>
        ...
        {loading ? (
          <div style={{ color:'#283484', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width:'100%' }}>
            <CircularProgress />
          </div>
        ) : error ? (
          <p>{error}</p>
